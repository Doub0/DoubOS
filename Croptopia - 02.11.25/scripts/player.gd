...
func _process(delta):
#	print("player item type",item_type)
	...
	if Input.is_action_just_pressed("1"):
		...
		if inv.slot_1_h_iax == true:
			...
			inv.slot_8_h_ch = false
			print("Axe in slot 1")
		if inv.item_held == "Construction Table":
			item_type = inv.item_held
			place(item_type)
...
func _on_load_radius_area_entered(area):
	var node = area.get_parent()
	print(node)
	node.process_mode = Node.PROCESS_MODE_ALWAYS
	
func _on_load_radius_area_exited(area):
	var node = area.get_parent()
	print(node)
	node.process_mode = Node.PROCESS_MODE_DISABLED

func player_can_move(variant:bool,variant2:bool):
	can_move = variant
	self.visible = variant2
	
	return can_move
func cam_preset(type:String):
	if type == "NORMAL":
		camera.zoom = Vector2(4,4.8)
		...
		camera.drag_horizontal_offset = -0.5
		camera.drag_vertical_offset = 0
